...
| 2026-06-01 | Leak 1 | **Closed.** Remaining 83 cloze cards confirmed intentionally kept — each isolates something the paired production card does not force | — |
...
| 2026-06-01 | Leak 5 | **Closed.** Deleted 5 European Spanish notes (*chulo*, *ser la caña*, *cuqui*, *majo*, *vale*); fixed *arriesgado* back (*ostras* → *uy*); fixed *coger el tren* back → *tomar el tren*, dropped Spain-register secondary example | — |
| 2026-06-01 | Leak 6 | **Phase 1 done.** Added `Interesting Facts` field to Production model; back template updated with collapsible `<details>` block. Phase 2 (migrate verbose backs) tracked in `leak6_interesting_facts_migration.md` — 249 notes, 13 batches of 20 | — |
| 2026-06-01 | Leak 6 | **Phase 2 complete.** All 244 Production notes reviewed; ~96 backs split into Back + Interesting Facts, ~148 clean. Notable: all 27 animal idiom backs had Spanish definitions moved to IF; grammar/etymology notes extracted from ~30 additional cards; 3 user annotation TODOs resolved inline (bagre front, irse de las manos front + pudiéramos note, lo vs le on cría cuervos). | See `leak6_interesting_facts_migration.md` |
| 2026-06-01 | Leak 4 (manos) | **Manual intervention.** Assessed the manos idiom batch (26 unseen cards) directly — found the material too dense, too boring, and hard to track as a block. Suspended many of the cards manually in Anki. Will unsuspend individually as each idiom becomes relevant or encountered in context. Confirms the reactive intake principle: batch-generating idioms ahead of exposure is low ROI; this batch is now parked rather than forced. | Manual Anki action; no script |
