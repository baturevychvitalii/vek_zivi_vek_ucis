...
The precedent already exists in the codebase: `plugins/anki-mcp` is an
independently-versioned package behind an integration boundary. Groves graduate toward
that same status — the grove format pillar is largely about making that graduation a
paved road.
