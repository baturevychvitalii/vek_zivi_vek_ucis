...
Parse the confirmed cards (strip display line numbers if present). Split each line on ` | ` → col1 (front/text), col2 (back), col3 (tags, space-separated).
...
try:
    ...
    # result["result"] is a list: note ID if added, null if duplicate/error
